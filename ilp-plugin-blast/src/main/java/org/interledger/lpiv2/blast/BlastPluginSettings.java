...
public interface BlastPluginSettings extends PluginSettings {
  ...
  static ImmutableBlastPluginSettings.Builder builder() {
    return ImmutableBlastPluginSettings.builder();
  }
   ...
   * @return
   */
  static ImmutableBlastPluginSettings.Builder applyCustomSettings(
      final ImmutableBlastPluginSettings.Builder builder, Map<String, Object> customSettings
  ) {
  ...
  /**
   * endpoint to POST packets to. If url contains a percent and the plugin is in `multi` mode, then the segment after
   * this plugin's own address will be filled where the `%` is  when routing packets.
   *
   * @return
   */
  ...
  @Value.Immutable
  @Modifiable
  abstract class AbstractBlastPluginSettings implements BlastPluginSettings {
